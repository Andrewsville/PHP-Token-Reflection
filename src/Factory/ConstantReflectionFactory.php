...
class ConstantReflectionFactory
{
	 ...
	 */
	public function createFromNode(Const_ $constantNode, Stmt $parentNode = NULL, $file)
	{
		$docComment = $constantNode->hasAttribute('comments') ? $constantNode->getAttribute('comments')[0] : NULL;
		...
		$constantReflection = new ConstantReflection(
			$constantNode->name,
			$constantNode->getAttribute('startLine'),
			$constantNode->getAttribute('endLine'),
			$docComment,
			$namespace,
			$annotations,
			$file
